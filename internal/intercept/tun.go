...
// ─── TUN 模式清理 ────────────────────────────────────────────
// sing-box 的 auto-route/auto-redirect 自建 tun0 与策略路由规则。tun0 消失后
...
// 列表（表名含 family，如 "inet sing-box"）。全部幂等。
func CleanupTun(tableIndex int, nftTables []string) error {
	...
	// sing-box 会按 rule_index 连续加多条规则，按表名匹配比固定 pref 列表更稳。
	for i := 0; i < 64; i++ {
	...
	for _, t := range nftTables {
		...
		if len(parts) == 2 { // "inet sing-box" / "ip sing-box_tproxy4"
